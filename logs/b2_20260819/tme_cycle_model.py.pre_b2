...
That is now false for B1 and has been since 2026-08-18:
  ...
  B2    no RTL.  Term written in the same style as B1's original projection,
        which turned out to be optimistic by T + 1 per (output row, template
        row).  Unmeasured in exactly the way B1's was.
...
-----------------------------------------------------------------
  ...
  architectural projection  B2 20.175, B0b 17.513998 / 17.806062 at II=1 and
                  II=3 on the count pass -- no RTL exists, and the II itself is
                  projected rather than measured.  B1 is now a warning about
                  both: its predicted tile term was optimistic by T+1 cycles
                  per (output row, template row), and B2/B0b's terms were
                  written in the same style and have not been measured either.
...
def main():
    ...
    if args.do_assert:
        ...
        print("frozen evidence re-proved: cosim x{}, {} board runs at {} clocks, "
              ...
              "B1 EXACT aggregate + phase-s-max delta, B2, "
